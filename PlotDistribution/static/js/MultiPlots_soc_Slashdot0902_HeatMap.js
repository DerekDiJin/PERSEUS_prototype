...
function drawPlot(divId, fileName, xLog, yLog, heatMap, xLabelName, yLabelName){
    ...
    var tooltip = d3.select(divId).append("div")
        .attr("class", "tooltip")
        .style("opacity", 0);
    d3.text("/static/data/soc-Slashdot0902_HeatMap/" + fileName, "text/csv", function(text) {
...
function clickShowKCenterAnomaly(gSize) {
  ...
  prevClickDotsSet = {};
  plotNames.forEach(function(pName){
    d3.text("/static/data/soc-Slashdot0902_HeatMap/kcenter/" + pName + "_anomaly.txt", "text/csv", function(text) {
        var data = d3.tsv.parseRows(text);
...
function clickShowAnomaly(gSize) {
  ...
  prevClickDotsSet = {};
  plotNames.forEach(function(pName){
    d3.text("/static/data/soc-Slashdot0902_HeatMap/" + gSize + "/" + pName + "_anomaly.txt", "text/csv", function(text) {
        var data = d3.tsv.parseRows(text);
        console.log("corresponding dots num: " + data.length);
        data.forEach(function(d) {
          // console.log(d);
