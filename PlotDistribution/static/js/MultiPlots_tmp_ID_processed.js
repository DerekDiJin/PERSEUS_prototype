...
var plotNames = ["degreeCount", "degreePagerank", "pagerankCount", "degreeRadius", "ev1ev2", "ev2ev3"];
...
svgMap["degreePagerank"] = drawPlot("#degreePagerank", "degreePagerank", true, true, false, "log(Degree)", "log(PageRank)");
svgMap["pagerankCount"] = drawPlot("#pagerankCount", "pagerankCount", true, true, false, "log(PageRank)", "log(Count)");
svgMap["degreeRadius"] = drawPlot("#degreeRadius", "degreeRadius", false, false, false, "src", "dst");
...
function drawPlot(divId, fileName, xLog, yLog, heatMap, xLabelName, yLabelName){
    ...
    var tooltip = d3.select(divId).append("div")
        .attr("class", "tooltip")
        .style("opacity", 0);
    d3.text("/static/data/tmp_ID_processed/" + fileName, "text/csv", function(text) {
...
function clickShowKCenterAnomaly(gSize) {
  ...
  prevClickDotsSet = {};
  plotNames.forEach(function(pName){
    d3.text("/static/data/tmp_ID_processed/kcenter/" + pName + "_anomaly.txt", "text/csv", function(text) {
        var data = d3.tsv.parseRows(text);
...
function clickShowAnomaly(gSize) {
  ...
  prevClickDotsSet = {};
  plotNames.forEach(function(pName){
    d3.text("/static/data/tmp_ID_processed/" + gSize + "/" + pName + "_anomaly.txt", "text/csv", function(text) {
        var data = d3.tsv.parseRows(text);
        console.log("corresponding dots num: " + data.length);
        data.forEach(function(d) {
          // console.log(d);
