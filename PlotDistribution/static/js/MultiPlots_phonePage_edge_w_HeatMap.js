...
var plotNames = ["degreeCount_HeatMap", "degreePagerank_HeatMap", "pageRankCount_HeatMap", "ev1ev2_HeatMap", "ev2ev3_HeatMap", "ev3ev4_HeatMap", "degreeUW_HeatMap", "pagerankUW_HeatMap", "ccCount_HeatMap"]; //"degreeRadius_HeatMap",
...
svgMap["degreeUW_HeatMap"] = drawPlot("#degreeUW", "degreeUW_HeatMap", true, true, true, "log(degreeU)", "log(degreeW)");
svgMap["pagerankUW_HeatMap"] = drawPlot("#pagerankUW", "pagerankUW_HeatMap", true, true, true, "log(pagerankU)", "log(pagerankW)");
svgMap["ccCount_HeatMap"] = drawPlot("#ccCount", "ccCount_HeatMap", true, true, false, "log(clusteringCoefficient)", "log(Count)");
...
var prevLinks = {},
    nodes = {};
var width_egonet = document.getElementById("egonet_svg").offsetWidth;
    height_egonet = 420;
...
var nodeId_button = document.getElementById("nodeId_button");
nodeId_button.onclick = function() {
...
function drawPlot(divId, fileName, xLog, yLog, heatMap, xLabelName, yLabelName){
    ...
    var tooltip = d3.select(divId).append("div")
      .attr("class", "tooltip")
      .style("opacity", 0);
      d3.text("/static/data/phonePage_edge_w_HeatMap/" + fileName, "text/csv", function(text) {
...
function handleResponse() {
    ...
    tableHeader.innerHTML = ' \
                      <th width="5%">NodeID</th> <th width="15%">Number</th> <th width="5%">Degree</th> <th width="5%">Degree(U)</th> <th width="11%">PageRank</th> <th width="13%">Eigenvector 1</th>\
                      <th width="13%">Eigenvector 2</th> <th width="13%">Eigenvector 3</th> <th width="10%">Egonet</th> <th width="10%">Adj Matrix</th>\
                      ';
    ...
    sampleNodes.forEach(function(n){
      ...
      var infoStr = '<td width="5%">' + n.nodeId + '</td>' + '<td width="15%">' + n.number + '</td>' + '<td width="5%">' + n.inoutdegree + '</td>' + '<td width="5%">' + n.inoutdegree_u + '</td>'   
     + '<td width="11%">' + pr.toExponential() + '</td>' + '<td width="13%">' + ev1.toExponential() + '</td>' + '<td width="13%">' + ev2.toExponential() + '</td>' 
     + '<td width="13%">' + ev3.toExponential() + '</td>' + '<td width="10%">' + '<button id="ego' + (n.nodeId).toString() +'">' + '<a>' + "Show" + '</a>' + '</button>' + '</td>' 
     + '<td width="10%">' + '<button id="adjMatrix' + (n.nodeId).toString() + '">' + '<a>'+"Show"+'</a>' + '</button>' + '</td>';
...
function clickShowAnomaly(gSize) {
  ...
  prevClickDotsSet = {};
  plotNames.forEach(function(pName){
    d3.text("/static/data/phonePage_edge_w_HeatMap/" + gSize + "/" + pName + "_anomaly.txt", "text/csv", function(text) {
        var data = d3.tsv.parseRows(text);
...
function deletenodes(nodeid) {
  ...
  var path = svg_egonet.append("svg:g").selectAll("path")
    ...
    .enter().append("svg:path")
      ...
      .attr("stroke-width", function(d) { return Math.log10(1+d.weight)}); 
...
function showEgonet(links) {
  ...
  node.append("circle")
      ...
      .on("mouseover", function(d) {
        ...
        console.log('in searchNumberOnly');
        if (window.XMLHttpRequest) {
            req = new XMLHttpRequest();
        } else {
            req = new ActiveXObject("Microsoft.XMLHTTP");
        }
        req.onreadystatechange = handleSearchNumberById;
        req.open("GET", "../PlotDistribution/searchNumberById/?nodeid=" + d.id, true);
        req.send();
        })
      // .on("mouseout", function(d) {
      //     document.getElementById("egonet_svg_info").innerHTML=""; /// color when mouse over
      //   })
      .on("click", function(d){
        if(!shift){
        console.log('enter click');
...
function handleSearchNumberById(){
  console.log("enter handleSearchNumberbyID")
  if (req.readyState != 4 || req.status != 200) {
    return;
  }
  var textData = req.responseText;
  console.log(textData);
  var responseJson = JSON.parse(textData)
  document.getElementById("egonet_svg_info").innerHTML=responseJson["number"]; /// color when mouse over
}
...
function searchById(nodeId){
  console.log('in searchById()');
  if (window.XMLHttpRequest) {
      req = new XMLHttpRequest();
  } else {
      req = new ActiveXObject("Microsoft.XMLHTTP");
  }
  req.onreadystatechange = handledSearchById;

  if (parseInt(nodeId) % 1 != 0){
    alert("The input format is not correct.");
    return;
  }

  req.open("GET", "../PlotDistribution/searchById/?nodeid=" + nodeId, true);
  req.send();
}

function handledSearchById(){
  console.log("enter handledSearchById")
  if (req.readyState != 4 || req.status != 200) {
      return;
  }
  var textData = req.responseText;
  ...
  plotNames.forEach(function(pName){
    ...
    res.forEach(function(d) {
      ...
      dot2Update.style("fill", "DarkBlue") //"rgb(0,231,231)")
              .attr("r", 4);
      prevClickAdjDotsSet[dot2Update.attr('id')] = dot2Update;
    });
...
function searchByNumber(number){
  ...
  req.onreadystatechange = handledSearchById;
  req.open("GET", "../PlotDistribution/searchByNumber/?number=" + number, true);
  req.send();
}
