...
function drawPlot(divId, fileName, xLog, yLog, heatMap, xLabelName, yLabelName){
    ...
    var tooltip = d3.select(divId).append("div")
        .attr("class", "tooltip")
        .style("opacity", 0);
    d3.text("/static/data/soc-Epinions1/" + fileName, "text/csv", function(text) {
      ...
      data.forEach(function(d) {
        ...
        if(origData == "5t6_230822116510728E-6"){
          console.log(origData);
          console.log(d);
          console.log(origDataMap[d]);
        }
      });
...
function clickShowAnomaly(gSize) {
  ...
  prevClickDotsSet = {};
  plotNames.forEach(function(pName){
    d3.text("/static/data/soc-Epinions1/" + gSize + "/" + pName + "_anomaly.txt", "text/csv", function(text) {
        var data = d3.tsv.parseRows(text);
