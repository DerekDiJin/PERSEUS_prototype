...
var plotNames = ["degreeCount_HeatMap", "degreePagerank_HeatMap", "pagerankCount_HeatMap", "ev1ev2_HeatMap", "ev2ev3_HeatMap", "ev3ev4_HeatMap", "fittingLine_HeatMap", "or_HeatMap"];
var origPlotNames = ["degreeCount", "degreePagerank", "pagerankCount", "ev1ev2", "ev2ev3", "ev3ev4"];
...
svgMap["netray_HeatMap"] = drawPlot("#netray", "netray_HeatMap", false, false, true, "src", "dst");
svgMap["fittingLine_HeatMap"] = drawPlot("#fittingLine", "fittingLine_HeatMap", true, false, false, "Eigenvector 3", "Eigenvector 4");
svgMap["or_HeatMap"] = drawPlot("#or", "or_HeatMap", true, false, false, "Eigenvector 3", "Eigenvector 4");
...
var showKDiv = document.getElementById("kcenter");
showKDiv.onclick = function() {
  clickShowKCenterAnomaly();
}
...
function drawPlot(divId, fileName, xLog, yLog, heatMap, xLabelName, yLabelName){
    ...
    var tooltip = d3.select(divId).append("div")
        .attr("class", "tooltip")
        .style("opacity", 0);
    d3.text("/static/data/soc-Slashdot0811_HeatMap/" + fileName, "text/csv", function(text) {
...
function clickShowKCenterAnomaly(gSize) {
  ...
  prevClickDotsSet = {};
  plotNames.forEach(function(pName){
    d3.text("/static/data/soc-Slashdot0811_HeatMap/kcenter/" + pName + "_anomaly.txt", "text/csv", function(text) {
        var data = d3.tsv.parseRows(text);
...
function clickShowAnomaly(gSize) {
  ...
  prevClickDotsSet = {};
  plotNames.forEach(function(pName){
    d3.text("/static/data/soc-Slashdot0811_HeatMap/" + gSize + "/" + pName + "_anomaly.txt", "text/csv", function(text) {
        var data = d3.tsv.parseRows(text);
        console.log("corresponding dots num: " + data.length);
        data.forEach(function(d) {
          // console.log(d);
