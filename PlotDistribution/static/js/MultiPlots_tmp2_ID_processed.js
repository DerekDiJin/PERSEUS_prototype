...
function drawPlot(divId, fileName, xLog, yLog, heatMap, xLabelName, yLabelName){
    ...
    var tooltip = d3.select(divId).append("div")
        .attr("class", "tooltip")
        .style("opacity", 0);
    d3.text("/static/data/tmp2_ID_processed/" + fileName, "text/csv", function(text) {
...
function clickShowAnomaly(gSize) {
  ...
  prevClickDotsSet = {};
  plotNames.forEach(function(pName){
    d3.text("/static/data/tmp2_ID_processed/" + gSize + "/" + pName + "_anomaly.txt", "text/csv", function(text) {
        var data = d3.tsv.parseRows(text);
